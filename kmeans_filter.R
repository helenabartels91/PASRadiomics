...
# Using an example feature set for testing purposes:
if(0){
	library(ISLR)
	x = na.omit(Hitters)
	x$Salary = NULL
	out = pca.kmeans.filter(x,k=NULL)
	out$k
	table(out$cluster) # assess balance across clusters
	cbind(out$cluster) # cluster label for each feature
	# Retrieve final medoids (i.e. features kept after filtering):
	names(x)[out$medoids]
}